...
const KapaliKasaForm: React.FC = () => {
  ...
  console.log('🚀 KapaliKasaForm location state:', location.state);
  ...
  const getFormTitle = () => {
    if (selectedBrand && selectedModel && selectedVariant) {
      return `${selectedBrand.name} ${selectedModel.name} ${selectedVariant.name} - Kapalı Kasa`;
    } else if (selectedBrand && selectedModel) {
      return `${selectedBrand.name} ${selectedModel.name} - Kapalı Kasa`;
    } else if (selectedBrand) {
      return `${selectedBrand.name} - Kapalı Kasa`;
    }
    return 'Kapalı Kasa İlanı Ver';
  };

  const getStepTitle = (step: number) => {
    const baseTitle = selectedBrand ? `${selectedBrand.name} Kapalı Kasa` : 'Kapalı Kasa';
    const stepTitles = [
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.usageArea) {
      confirm({
        title: 'Eksik Bilgi',
        description: 'Kullanım alanı seçimi zorunludur.',
        severity: 'warning'
      });
      return;
    }
    ...
    try {
      console.log('🚀 KapaliKasa form submission starting...');
      ...
      const payload = createStandardPayload({
        ...
        // KapaliKasa specific properties
      ...
      if (response.success) {
        console.log('✅ KapaliKasa listing created successfully:', response.data);
        
        await confirm({
          title: '🎉 Başarılı!',
          description: 'Kapalı Kasa ilanınız başarıyla oluşturuldu! Admin onayından sonra yayınlanacaktır. İlanlarım sayfasından durumunu takip edebilirsiniz.',
          severity: 'success'
        });
      ...
      console.error('❌ KapaliKasa listing creation error:', err);
      confirm({
        title: 'Hata',

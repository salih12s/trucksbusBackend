...
const KayaTipiForm: React.FC = () => {
  ...
  console.log('🚀 KayaTipiForm location state:', location.state);
  ...
  const [userInfo, setUserInfo] = useState({
    ...
    const [formData, setFormData] = useState<KayaTipiFormData>({
    title: '',
    description: '',
    productionYear: '',
    length: '',
  ...
  const getFormTitle = () => {
    if (selectedBrand && selectedModel && selectedVariant) {
      return `${selectedBrand.name} ${selectedModel.name} ${selectedVariant.name} - Kaya Tipi`;
    } else if (selectedBrand && selectedModel) {
      return `${selectedBrand.name} ${selectedModel.name} - Kaya Tipi`;
    } else if (selectedBrand) {
      return `${selectedBrand.name} - Kaya Tipi`;
    }
    return 'Kaya Tipi İlanı Ver';
  };

  const getStepTitle = (step: number) => {
    const baseTitle = selectedBrand ? `${selectedBrand.name} Kaya Tipi` : 'Kaya Tipi';
    const stepTitles = [
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log('🚀 KayaTipi form submission starting...');
      ...
      if (response.success) {
        console.log('✅ KayaTipi listing created successfully:', response.data);
        
        await confirm({
          title: '🎉 Başarılı!',
          description: 'Kaya Tipi ilanınız başarıyla oluşturuldu! Admin onayından sonra yayınlanacaktır. İlanlarım sayfasından durumunu takip edebilirsiniz.',
          severity: 'success'
        });
      ...
      console.error('❌ KayaTipi listing creation error:', err);
      confirm({
        title: 'Hata',
  ...
  const handleInputChange = (field: keyof KayaTipiFormData, value: any) => {

...
interface KapakliKayaTipiFormData {
  // Temel Bilgiler
  title: string;
  description: string;
  price: string;
  ...
  // Kaya Tipi'ne özel - Devrilme Yönleri
  devrilmeArkaya: boolean;
  devrilmeSaga: boolean;
  devrilmeSola: boolean;
...
const KapakliKayaTipiForm: React.FC = () => {
  ...
  console.log('🗻 KapakliKayaTipiForm yüklendi! variantId:', variantId);

  const [formData, setFormData] = useState<KapakliKayaTipiFormData>({
    title: '',
    description: '',
    price: '',
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      case 0: // Kapaklı(Kaya Tipi) Bilgileri
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Devrilme yönlerini topla
      const devrilmeYonleri = [];
      if (formData.devrilmeArkaya) devrilmeYonleri.push('Arkaya');
      if (formData.devrilmeSaga) devrilmeYonleri.push('Sağa');
      if (formData.devrilmeSola) devrilmeYonleri.push('Sola');

      // API için veri hazırlığı - Kapaklı(Kaya Tipi)'yi Kuruyük kategorisi altında gönder
      const listingData = {
        ...
        // Kapaklı(Kaya Tipi)'ya özel bilgileri properties olarak gönder
        properties: {
          dorseType: 'Kapaklı Kuruyük (Kaya Tipi)',
          ...
          devrilmeYonleri: devrilmeYonleri.length > 0 ? devrilmeYonleri.join(', ') : 'Yok',
          warranty: formData.warranty ? 'Evet' : 'Hayır',
          negotiable: formData.negotiable ? 'Evet' : 'Hayır'
        }
      };

      console.log('🚀 Kapaklı(Kaya Tipi) Kuruyük ilanı oluşturuluyor...', listingData);

      // API çağrısı
      const response = await api.post('/listings', listingData);
      
      if (response.data) {
        console.log('✅ Kapaklı(Kaya Tipi) Kuruyük ilanı başarıyla oluşturuldu:', response.data);
        alert('İlanınız başarıyla oluşturuldu! Admin onayından sonra yayınlanacaktır.');
        navigate('/');
      }
    } catch (err: any) {
      console.error('❌ Kapaklı(Kaya Tipi) Kuruyük ilanı oluşturma hatası:', err);
        ...
        return (
          <Stack spacing={3}>
            <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              Kapaklı(Kaya Tipi) Kuruyük Dorse Bilgileri
            </Typography>
            ...
            <TextField
              ...
              placeholder="Örn: 2018 Model Kapaklı(Kaya Tipi) Kuruyük Dorse"
              required
            />
            ...
            <TextField
              ...
              placeholder="Kapaklı(Kaya Tipi) kuruyük dorse aracınız hakkında detaylı bilgi verin..."
              required
            />
          </Stack>
        );
      ...
      case 1: // Teknik Özellikler
        return (
          <Stack spacing={3}>
            ...
            {/* Kaya Tipi'ne özel - Devrilme Yönü */}
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 2, fontWeight: 'bold' }}>
                Devrilme Yönü (Kaya Tipi Özelliği)
              </Typography>
              <Box sx={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={formData.devrilmeArkaya}
                      onChange={(e) => handleInputChange('devrilmeArkaya', e.target.checked)}
                    />
                  }
                  label="Arkaya"
                />
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={formData.devrilmeSaga}
                      onChange={(e) => handleInputChange('devrilmeSaga', e.target.checked)}
                    />
                  }
                  label="Sağa"
                />
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={formData.devrilmeSola}
                      onChange={(e) => handleInputChange('devrilmeSola', e.target.checked)}
                    />
                  }
                  label="Sola"
                />
        ...
        return (
          <Stack spacing={3}>
            ...
            <Typography variant="body2" color="text.secondary">
              Kapaklı(Kaya Tipi) kuruyük dorse aracınızın fotoğraflarını yükleyin (Maksimum 15 adet)
            </Typography>
      ...
      <Container maxWidth="lg" sx={{ py: 4, mt: 4 }}>
        ...
        <Box sx={{ mb: 4, textAlign: 'center' }}>
          <Typography variant="h4" component="h1" gutterBottom>
            Kapaklı(Kaya Tipi) Kuruyük İlanı Oluştur
          </Typography>
          <Typography variant="h6" color="primary" sx={{ mb: 2 }}>
            Dorse - Kuruyük - Kapaklı(Kaya Tipi)
          </Typography>
        </Box>

...
interface KapaksızPlatformFormData {
  // Temel Bilgiler
  title: string;
  description: string;
  price: string;
  ...
  // Kuruyük Kapaksız(Platform) Özel Bilgiler
  dingilSayisi: number;
  uzunluk: number; // metre
  genislik: number; // metre
  kapakYuksekligi: number; // metre (platform için de kullanılabilir)
...
const steps = [
  'Kapaksız(Platform) Bilgileri',
  'Teknik Özellikler',
  'Fotoğraflar',
  'İletişim & Fiyat'
];

const KapaksızPlatformForm: React.FC = () => {
  const { user } = useAuth();
  ...
  console.log('🏗️ KapaksızPlatformForm yüklendi! variantId:', variantId);

  const [formData, setFormData] = useState<KapaksızPlatformFormData>({
    title: '',
    description: '',
    price: '',
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      case 0: // Kapaksız(Platform) Bilgileri
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // API için veri hazırlığı - Kapaksız(Platform)'u Kuruyük kategorisi altında gönder
      const listingData = {
        ...
        // Kapaksız(Platform)'a özel bilgileri properties olarak gönder
        properties: {
          dorseType: 'Kapaksız Kuruyük (Platform)',
      ...
      console.log('🚀 Kapaksız(Platform) Kuruyük ilanı oluşturuluyor...', listingData);

      // API çağrısı
      const response = await api.post('/listings', listingData);
      
      if (response.data) {
        console.log('✅ Kapaksız(Platform) Kuruyük ilanı başarıyla oluşturuldu:', response.data);
        alert('İlanınız başarıyla oluşturuldu! Moderatör onayından sonra yayınlanacaktır.');
        navigate('/');
      }
    } catch (err: any) {
      console.error('❌ Kapaksız(Platform) Kuruyük ilanı oluşturma hatası:', err);
        ...
        return (
          <Stack spacing={3}>
            <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              Kapaksız(Platform) Kuruyük Dorse Bilgileri
            </Typography>
            ...
            <TextField
              ...
              placeholder="Örn: 2018 Model Kapaksız(Platform) Kuruyük Dorse"
              required
            />
            ...
            <TextField
              ...
              placeholder="Kapaksız(Platform) kuruyük dorse aracınız hakkında detaylı bilgi verin..."
              required
            />
          </Stack>
        );
        ...
        return (
          <Stack spacing={3}>
            ...
            <Typography variant="body2" color="text.secondary">
              Kapaksız(Platform) kuruyük dorse aracınızın fotoğraflarını yükleyin (Maksimum 15 adet)
            </Typography>
      ...
      <Container maxWidth="lg" sx={{ py: 4, mt: 4 }}>
        ...
        <Box sx={{ mb: 4, textAlign: 'center' }}>
          <Typography variant="h4" component="h1" gutterBottom>
            Kapaksız(Platform) Kuruyük İlanı Oluştur
          </Typography>
          <Typography variant="h6" color="primary" sx={{ mb: 2 }}>
            Dorse - Kuruyük - Kapaksız(Platform)
          </Typography>
        </Box>
...
export default KapaksızPlatformForm;

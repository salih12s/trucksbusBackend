...
interface PlatformRomorkFormData {
...
const PlatformRomorkForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<PlatformRomorkFormData>({
  ...
  const getFormTitle = () => {
    ...
    return 'Platform Römorku İlanı Ver';
  };
  ...
  const getStepTitle = () => {
    ...
    return 'Platform Römorku İlan Detayları';
  };
  ...
  useEffect(() => {
    const loadCities = async () => {
      try {
        ...
        console.log('🏙️ PlatformRomorkForm: Şehirler yüklendi:', citiesData.length);
      } catch (error) {
        console.error('❌ PlatformRomorkForm: Şehirler yüklenemedi:', error);
      } finally {
  ...
  const handleCityChange = async (cityId: string, cityName: string) => {
    try {
      ...
      console.log('🏘️ PlatformRomorkForm: İlçeler yüklendi:', districtsData.length);
    } catch (error) {
      console.error('❌ PlatformRomorkForm: İlçeler yüklenemedi:', error);
    } finally {
  ...
  useEffect(() => {
    if (user && cities.length > 0) {
      console.log('👤 PlatformRomorkForm: User data loading:', user);
      setFormData(prev => ({
        ...prev,
  ...
  const handleInputChange = (field: keyof PlatformRomorkFormData, value: any) => { setFormData(prev => ({ ...prev, [field]: value })); };
  ...
  const handleSubmit = async () => {
    ...
    console.log('PlatformRomorkForm - Form gönderim başlatıldı');
    ...
    try {
      ...
      if (response.success) {
        console.log('✅ Platform Römorku ilanı başarıyla oluşturuldu:', response.data);
        await confirm({
          title: 'Başarılı',
      ...
      case 0: return (
        <Card>
          <CardContent>
            ...
            <FormControlLabel 
              control={<Checkbox checked={formData.hasDamper} onChange={(e) => handleInputChange('hasDamper', e.target.checked)} />} 
              label="Damperli mi?" 
            />
            <FormControl>
              <FormLabel>Takas Kabul Ediyor musunuz?</FormLabel>
              <RadioGroup value={formData.isExchangeable} onChange={(e) => handleInputChange('isExchangeable', e.target.value)}>
...
export default PlatformRomorkForm;

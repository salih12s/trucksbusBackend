...
const VasitaRomorkForm: React.FC = () => {
  ...
  useEffect(() => {
    const loadCities = async () => {
      try {
        ...
        console.log('🏙️ VasitaRomorkForm: Şehirler yüklendi:', citiesData.length);
      } catch (error) {
        console.error('❌ VasitaRomorkForm: Şehirler yüklenemedi:', error);
      } finally {
  ...
  const handleCityChange = async (cityId: string, cityName: string) => {
    try {
      ...
      console.log('🏘️ VasitaRomorkForm: İlçeler yüklendi:', districtsData.length);
    } catch (error) {
      console.error('❌ VasitaRomorkForm: İlçeler yüklenemedi:', error);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    console.log('🔍 VasitaRomorkForm Debug Info:', {
      selectedBrand,
      selectedModel,
      selectedVariant,
  ...
  return (<Box sx={{ width: '100%', p: 3 }}><Typography variant="h4" component="h1" gutterBottom>
    {selectedBrand && selectedModel && selectedVariant 
      ? `${selectedBrand.name} ${selectedModel.name} ${selectedVariant.name} - Vasıta Römorku İlanı Ver`
      : 'Vasıta Römorku İlanı Ver'
    }

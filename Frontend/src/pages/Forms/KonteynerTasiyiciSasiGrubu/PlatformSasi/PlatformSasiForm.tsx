...
const PlatformSasiForm: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.log('🔍 Vehicle type:', (location.state as any)?.vehicleType);
      console.log('🔍 Brand:', (location.state as any)?.brand);
      console.log('🔍 Model:', (location.state as any)?.model);
      console.log('🔍 Variant:', (location.state as any)?.variant);
      
      // Location state'den değerleri al
      const locationState = location.state as any;
      const vehicleType = locationState?.vehicleType;
      const brand = locationState?.brand;
      const model = locationState?.model;
      const variant = locationState?.variant;
      ...
      const payload = {
        ...
        category_id: vehicleType?.categories?.id || brand?.vehicle_types?.categories?.id || "vehicle-category-001",
        brand_id: brand?.id || null,
        model_id: model?.id || null,
        variant_id: variant?.id || null,
        vehicle_type_id: vehicleType?.id || brand?.vehicle_type_id || null,
      ...
      if (response.success) {
        await confirm({ 
          title: 'Başarılı',
          description: 'İlanınız başarıyla oluşturuldu ve admin onayı için gönderildi.',
          severity: 'success'
        });
        navigate('/');
      } else {
        throw new Error(response.message || 'İlan oluşturulurken bir hata oluştu');
      }
    } catch (error: any) {
      console.error('Error submitting form:', error);
      await confirm({ 
        title: 'Hata',
        description: error.message || 'İlan oluşturulurken bir hata oluştu. Lütfen tekrar deneyin.',
        severity: 'error'
      });
    }
  };

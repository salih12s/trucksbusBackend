...
const TarimTankerForm: React.FC = () => {
  ...
  const getFormTitle = () => {
    ...
    if (selectedModel?.model_name) {
      return `${selectedModel.model_name} Tanker İlanı`;
    }
    if (selectedBrand?.brand_name) {
      return `${selectedBrand.brand_name} Tanker İlanı`;
    }
    return 'Tarım Tanker İlanı Ver';
  };

  const getStepTitle = () => {
    const baseName = selectedVariant?.variant_name || selectedModel?.model_name || 'Tarım Tanker';
  ...
  const handleInputChange = (field: keyof TarimTankerFormData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!selectedBrand) {
        console.error('Brand seçimi bulunamadı! location.state:', location.state);
        await confirm({
          title: 'Eksik Bilgi',
